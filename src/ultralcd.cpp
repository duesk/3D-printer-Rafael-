...
void kill_screen(const char* lcd_msg) {
  ...
  inline void line_to_z(const float &z) {
    ...
    void lcd_sdcard_pause() {
      enqueue_and_echo_commands_P(PSTR("G1 X-160 Y-90"));
      //enqueue_and_echo_command("G1 X-165 Y-100");
      card.pauseSDPrint();
    ...
    void lcd_sdcard_stop() {
      ...
      lcd_return_to_status();
      enqueue_and_echo_commands_P(PSTR("G1 X0 Y0 Z180"));
    }
...
*/
  ...
  int setTemp (){
    ...
    return thermalManager.target_temperature[0];
  }

  void lcd_main_menu() {
    START_MENU();

    int valor = thermalManager.target_temperature[0];
    if(valor == 0){
      thermalManager.target_temperature[0]=190;
    }

    //TODO Inicio de menu

    MENU_BACK(MSG_WATCH); ////// regresar al inicio 

    
    MENU_MULTIPLIER_ITEM_EDIT_CALLBACK(int3, MSG_NOZZLE, &thermalManager.target_temperature[0], 100, HEATER_0_MAXTEMP - 15, watch_temp_callback_E0);
...
/*
   ...
   // }
    ...
    //SERIAL_ECHOPGM(" temperatura seteada");
    //SERIAL_ECHOPAIR(" temperatura seteada",thermalManager.target_temperature[0]);
   ...
   *
   */
  void _lcd_prehPeat(const int16_t endnum, const int16_t temph, const int16_t tempb, const int16_t fan) {
    if (temph > 0) thermalManager.setTargetHotend(min(heater_maxtemp[endnum], temph), endnum);
    #if TEMP_SENSOR_BED != 0
      if (tempb >= 0) thermalManager.setTargetBed(tempb);
    #else
  ...
  void lcd_cooldown() {
    ...
    void _lcd_ubl_output_map_lcd() {
      ...
      #if IS_KINEMATIC
        n_edit_pts = 9; // Delta accessible edit points
      #else
